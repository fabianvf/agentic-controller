...
# Target the dev cluster when invoked via `make dev-probe`, which passes
# DEV_KUBECONFIG. Without this, kubectl silently falls through to
# ~/.kube/config -- i.e. whatever cluster you happen to be pointed at -- and
# this script CREATES PODS.
#
# Deliberately conditional: running this script directly against another
# cluster (a real OpenShift, say) is a supported and important use, so an
# explicitly-set KUBECONFIG with no DEV_KUBECONFIG is honoured as-is.
...
collect_cluster_facts() {
    ...
    if ! kubectl get --raw /version >/dev/null 2>&1; then
        ...
        err "For the local dev cluster run:  make dev-probe"
        exit 1
    fi
...
main() {
    ...
    if [ -n "${sandbox}" ] && kube get pod "${sandbox}" >/dev/null 2>&1; then
        ...
        log "  (run 'make dev-apply && make dev-run' first to compare against a real agent pod)"
    fi
